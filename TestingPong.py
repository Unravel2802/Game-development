...
#		Center ball collision check
		if math.sqrt(math.pow(balls[1].xpos-self.xpos,2)+math.pow(balls[1].ypos-self.ypos,2))<=self.size+balls[1].size:
...
#				self.xvel=self.xvel*-1
				...
				if players[1].score>players[0].score:
					paddles[1].erase()
					paddles[1].height=int(sheight*0.1)
					if paddles[0].height==int(sheight*0.1):
						paddles[0].erase()
						paddles[0].height=int(sheight*0.2)
...
#				self.xvel=self.xvel*-1
				...
				if players[1].score<players[0].score:
					paddles[0].erase()
					paddles[0].height=int(sheight*0.1)
					if paddles[1].height==int(sheight*0.1):
						paddles[1].erase()
						paddles[1].height=int(sheight*0.2)	
...
#		Check if it collide with the ball in the middle of the screen	
...
# key handling
			...
			if akey:
						paddles[0].yvel=paddles[0].yvel+int(-1*sheight*.5)
			if zkey:
						paddles[0].yvel=paddles[0].yvel+int(sheight*.5)
			if upkey:
						paddles[1].yvel=paddles[1].yvel+int(-1*sheight*.5)
			if downkey:
						paddles[1].yvel=paddles[1].yvel+int(sheight*.5)
